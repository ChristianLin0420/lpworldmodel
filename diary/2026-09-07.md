...
**P5 remains uninformative**, but for a new reason: the treatment degrades the action channel
(0.118-0.236) while its control keeps it (0.286-0.357), so the pair differs by more than the
pairing the control was built to isolate. The degradation cannot come from the state entering
the code -- in `aux` mode it provably does not -- so it comes from the objective's gradients
reshaping the shared ViT weights.

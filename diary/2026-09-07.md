...
![the premise](assets/2026-09-07/arch-premise.svg)
...
Each figure is embedded beside the method it describes, above. They are **paper-style
architecture diagrams**, not the round-8 layout: no shared schematic band, no results panels
— the clip is unrolled left to right so the temporal state is a visible arrow between frames,
and every equation sits in its own reserved box rather than floating over a connector.

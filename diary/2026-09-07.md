...
2. That drift is a **first-class instrument**, `enc_state_gap`, emitted every step by every
   stateful arm and **read before the success rate** (§6).

   > **RETRACTED, 2026-09-08. The shipped metric is wrong and every value it logged is
   > invalid.** `visual_world_model.py:2095` computes `state.solo(u_emb)`, but `u_emb` has
   > already been through `forward_state` and therefore already carries `x + C s`. So the
   > metric applies the state operator **twice** and compares the result against `z_emb`.
   > It is not the single-frame limit and never was. The drift itself is real — measured
   > directly on checkpoints, live arms sit at **1.16–1.26** relative at t ≥ 1 while the
   > frozen controls sit at exactly **0.0000** — but that was established by re-measuring
   > from the checkpoints, not by this instrument. The correct quantity needs the
   > *pre-state* per-frame code, which `encode_obs` does not return; computing it means
   > returning that tensor too.
...
**Measured cost, CPU, 3 steps after warmup at batch 2** (CPU overstates Python-loop overhead,
so this is conservative for the scan and honest about the rest):

| arm | s/step | × baseline |
|---|---|---|
| baseline | 3.26 | 1.00 |
| P1 `enc-ssm` | 3.01 | **0.92** — free, within noise |
| P5 `state-sinv` | 3.70 | 1.14 |
| **P3 `enc-scan`** | 13.96 | **4.29** |

So P1 and the objectives are essentially free, and **P3/P4 are the round's compute**. They are
launched with a much longer window chain (§5); tail windows exit immediately once `DONE`
exists, so over-provisioning costs nothing, but the arms themselves are real spend and are the
first thing to cancel if the round needs to shrink.
...
**Window budget follows the measured cost, not a guess.** P1 is free (0.92× baseline) and the
objectives are near-free (1.14×), but the scan is **4.29×**, so:

| arms | windows |
|---|---|
| baseline, P1, P2, P5, P6, P7 and their controls | 6 |
| P3, P4 and their controls | **16** |

Over-provisioning is safe by design — `submit_until_done.sh`'s header records that a window
exits immediately when the `DONE` sentinel exists, so the tail of a chain costs nothing — but
the scan arms are genuinely expensive and are **the first thing to cancel** if the round needs
to shrink: they are 24 of the 87 runs and the majority of its GPU-hours.
...
| every other test file | **516 passed, 1 skipped, 0 failed** |

**One regression I introduced and fixed.** `tests/test_arms.py` caches a built D=384 model per
arm; 13 new arms pushed the file past the **24 GB cgroup limit** and it was SIGKILLed partway
through, which reads as a hang rather than a memory bug. The cache is now capped, which is
transparent — dropping a dict entry leaves a caller's reference valid, so a miss costs only
rebuild time.

**And one bug of my own, caught by that file.** The round-9 tests appended a helper named
`_fresh`, which already existed at `tests/test_arms.py:534` returning a five-tuple. The later
definition wins at import, so **eight unrelated T4/T6 tests** started failing with
`cannot unpack non-iterable VWorldModel object` — nothing to do with the cache, which is what
I first blamed. Renamed to `_r9_build`. It is the same class of silent collision this repo
already documents for `resolve_arm` and for `mup.py`'s substring matching.
...
instruction is to submit them together.

### 8.1 Launched

All 87 runs are in the queue with their evaluations attached.

| | |
|---|---|
| runs submitted | **87** (42 pushT, 45 wall) |
| evals chained to their final window | **87** |
| training windows queued | 762 |
| submission errors | 0 |

Monitor: `scripts/watch/wave32_autopilot.sh`. It is a **safety net, not the primary path** —
evals are already chained at submit time — and it exists for the two ways that chain breaks:
a chain that exhausts its windows without writing `DONE` (the eval then fires, finds no
`DONE`, and exits 1 by design), and a window preempted in a way that kills the dependency.
It reports **success rate only**: arm, n on shared seeds, ΔSR against its own control, and
ΔSR against the baseline in the same environment.

Round 8's survivors (T3 `tjepa`, ST1 `st-tube`) continue undisturbed under `b32541si1`.
...
## 10. What round 9 actually found

**The proposals do not fail because the state is unconstrained, or because the code collapses,
or because the goal is mis-encoded. They fail because the state hands the predictor a
shortcut that lets it minimise the loss without using the action.**

`causal/d_action` — the RMS displacement of the predicted latent when the batch's actions are
permuted, i.e. precisely the quantity CEM optimises over — separates the round perfectly, with
no overlap:

| group | `d_action` | SR |
|---|---|---|
| baseline + **every frozen control** (n = 15) | **0.276 – 0.393** | 0.350 – 0.673 |
| **every live-state arm** (n = 18) | **0.0009 – 0.128** | 0.000 – 0.053 |

A live state costs ~**100×** of the model's action-sensitivity. CEM then searches actions
against a predictor whose output barely moves when the action changes, so a success rate of
zero is the arithmetic consequence, not a surprise.

**This trap is already documented in this repo.** `conf/train_rdmreg.yaml:206`: *"The collapse
mode is d_action → 0 (a predictor that ignores the action inside the ball)."* Round 9 walked
into it because the round was designed around a *representation* question and never asked what
the intervention would do to the *action channel*.

**Why the shortcut exists, structurally.** With `s_{t+1} = A s_t + Bz x_{t+1}` and
`out_t = x_t + C s_t`, the component `A s_t` is already determined by `z_t`. The predictor can
lower `z_loss` by propagating that deterministic part; the action is not needed for it. The
measured signature follows exactly:

* one-step fit **~2× better** than the frozen control (epoch-matched: 0.0045 vs 0.0091);
* multi-step error exploding — h2/h1 growth **40–96×** against the baseline's 1.9–2.9;
* `ρ(A)` drifting from its 0.9 init to **1.0–2.6**;
* `effective_dim` *rising* to 34.8–40.8 (baseline 24–26) — so this is **not** code collapse.

**The purest case is P2.** `enc-deep` predicts *better than the baseline at every horizon*
(h1 0.0011, h2 0.0033, h4 0.0111 vs 0.0055 / 0.0133 / 0.0438) and still plans at **0.000**. A
world model can forecast the future excellently and be worthless for control, if it does so by
ignoring the actions.

### 10.1 A correction to §0, and to the round's framing

§0 replaced "the goal was never produced in training" with "the *targets* drifted." Both are
descriptions of the same drift, and neither is the operative cause. The drift is real, but the
frozen controls prove it is not what kills planning on its own: what kills planning is that the
drift is *predictable from the previous frame*, which is what lets the predictor drop the
action. The round's own §2.0 contract — `A = 0` reproduces the baseline — is what makes this
readable, because the frozen arms hold `d_action` at 0.276–0.393 and plan normally.

### 10.1b TWO failure modes, not one — and P3's control is the second one

§10 above explains P1, P2 and P5–P7. It does **not** explain P3 and P4, and a prediction I
registered before checking got this wrong, which is worth recording.

The prediction: P3's scan runs over **tokens within a frame**, not over frames, so it carries
no cross-frame state and the action shortcut cannot apply to it — therefore P3's `d_action`
should survive. Measured, it does not:

| arm | frame state | token mixing | `d_action` |
|---|---|---|---|
| baseline, frozen frame-state controls | — | attention | **0.276 – 0.393** |
| `enc-scan` (scan active) | none | scan | **0.0000 – 0.0008** |
| `enc-scan-frozen` (the control, `a = 0`) | none | **none** | **0.0000 – 0.0009** |

**The control is degenerate, and that is my error.** §2.3 described `a = 0` as degenerating to
"a per-token MLP" and treated that as an acceptable control isolating *token mixing*. But with
`a = 0` the sweep returns its input unchanged, so the block becomes `scan_out(2·u)` applied
independently per token — the encoder has **no token mixing at any depth**, twelve blocks of
per-patch MLP. That cannot build a usable representation, and `d_action ≈ 0` is the expected
result rather than an informative one.

Consequences:

* P3's intended contrast (`enc-scan` vs `enc-scan-frozen`) compares a possibly-fine encoder
  against a broken one and cannot isolate "scan instead of attention".
* The only honest comparison for P3/P4 is against the **baseline**, i.e. against attention.
* So the round has two distinct failure modes: **P1/P2/P5/P6/P7** die of the action-free
  shortcut; **P3/P4** die of losing attention, with a control that removed too much to say
  whether the scan itself was at fault.

A usable control for a token-mixing operator has to *keep* mixing while changing its form —
attention itself is that control, and `a = 0` is not.

### 10.2 Five of the seven could not have returned a readable result

P5–P7 attach to P1's state; ST1 (round 8) attaches to `block_causal`. Both substrates plan at
0.000, and in both cases the *control* inherits the same substrate. So the objective contrasts
compare two dead arms — P6 lands at exactly `0.000 − 0.000` at n = 3. The design error is the
same one twice: **an objective was layered on an architecture already known to plan at zero, on
the theory that the architecture failed for want of a demand.** It did not.

**What a fair test of P5–P7 would require:** an encoder whose `d_action` survives. Bounding
`‖A‖` is not enough — the shortcut exists for any `A ≠ 0`, because the predictable component is
`A s_t` itself. The constraint has to fall on what the predictor may use, not on how large the
state is.

---

## 11. Round 9 repaired — the two design errors, fixed and relaunched

Both errors were mine, both are measured in §10, and both made an arm unreadable rather than
merely unsuccessful.

### 11.1 The scan never mixed anything (§10.1b)

`ScanMixer` initialised `scan_a = 0`, and `a = tanh(scan_a)`, so `a = 0` and the sweep returned
its input unchanged. The treatment therefore *started* as twelve blocks of per-patch MLP, and
its "frozen control" stayed one permanently. Measured `d_action` for both: **0.0000-0.0009**,
against 0.276-0.393 for anything that mixes tokens at all.

**Fix.** `enc_scan_a` (default **0.5**) is the decay the sweep starts from, so the operator
mixes from step 0; `enc_scan_freeze` now freezes the decay *at that value*, so the control still
mixes and the contrast isolates **learning** the mixing rather than deleting it. The
scan-versus-attention question is answered against the baseline, which is its only honest
control.

### 11.2 The objectives were mounted on a substrate that cannot plan (§10)

In `add` mode the code carries `C s_t`, and `s_{t+1} = A s_t + Bz x_{t+1}`, so the next code is
largely determined by the current one. The predictor lowers `z_loss` by propagating that
instead of using the action, `d_action` collapses ~100x, and CEM has nothing to optimise.
Treatment and control both plan at zero — P6 landed at exactly `0.000 - 0.000` at n = 3.

**Fix, in two parts, because the three objectives are not alike:**

* **P6 and P7 never read the state.** They constrain `z_emb`. The blanket assertion that they
  required `enc_ssm=True` was the error: its only effect on them was to poison `z_emb`. They now
  run on the **stock encoder**, where `d_action` is healthy.
* **P5 genuinely needs a state**, so it gets `enc_ssm_out=aux`: the code handed to the predictor
  is the per-frame code **bit-for-bit**, and the state is exposed only to P5's own loss. No
  shortcut can exist, because the prediction path never sees the state. Unlike the `C = 0`
  contract, this holds for **all of training**, not just at init — `A`, `Bz` and `C` never touch
  the output. `C` starts at the identity there rather than zero, or the readout would be
  identically zero and the objective would have nothing to constrain.

### 11.3 The relaunch

Ten arms x 2 environments x 3 seeds = **60 runs**, evaluations chained to their final window.

| | arm | control | substrate |
|---|---|---|---|
| P3 | `scan2` | `scan2-fixed` (decay frozen, **still mixes**) | — |
| P4 | `st2` | `st2-fixed` | the two repaired halves composed |
| P5 | `sinv2` | `sinv2-shuf` | `enc_ssm_out=aux` |
| P6 | `vel2` | `sum2` | stock encoder |
| P7 | `nce2` | `nce2-shuf` | stock encoder |

**Kept from the first attempt, because they are the finding rather than a defect:**
`PiWM-enc-ssm` and `PiWM-enc-deep` with their frozen controls. **Cancelled:** the superseded
pushT arms of the first attempt, and `PiWM-enc-scan`'s run directories, which held checkpoints
trained under the old `a = 0` scan and would otherwise have been resumed under the new default.

**A watcher caught me out during the changeover.** `wave32_autopilot.sh` re-extended
`PiWM-enc-scan` minutes after it was cancelled — its own header demands "env-scoped arm lists,
so a cancellation stays cancelled", and the lists were simply stale. Superseded arms must never
appear in a live watcher's `ARMS`. `scripts/watch/wave34_autopilot.sh` replaces it and names
only the arms that are still wanted.

### 11.4 What a null means now -- and the limit of the gate

The intent: `d_action` is the pre-outcome read, so a repaired arm inside the healthy band that
still plans at zero has been genuinely tested, while one below the band has not.

**That is half true, and the half that fails is the half I stated first.** Measured on the
repaired wall runs, four seeds sit inside the band and still plan at 0.04-0.18:

| run | `d_action` | SR |
|---|---|---|
| `sinv2-shuf` s4 | 0.328 | 0.060 |
| `sinv2-shuf` s5 | 0.286 | 0.040 |
| `vel2` s4 | 0.293 | 0.060 |
| `vel2` s5 | 0.288 | 0.180 |

So the honest form of the rule is **one-directional**:

* `d_action` near zero (the first attempt's 0.0009-0.13) **explains** a failure -- CEM is
  optimising against a predictor that barely responds to the action, and no other cause need be
  sought;
* `d_action` in the healthy range **does not predict success**. Across 0.28-0.40 the success
  rate ranges from 0.04 to 0.80.

A weaker signal may survive -- everything that plans well sits above 0.36 (baseline 0.379-0.400,
`sum2` 0.361-0.422) while the counterexamples sit at the band's floor -- but that is four points
and is not worth a threshold. Use `d_action` to *disqualify* an arm from interpretation, never to
qualify one.

### 11.5 First readable results from the repaired round (wall)

**P6 is refuted ON WALL, is NOT testable on pushT, and the reason it fails is a measured
trade-off rather than a null.**

On wall both arms are healthy and the contrast is clean. On pushT the CONTROL is destroyed too
-- `sum2` plans at 0.08-0.12 against a baseline of 0.24-0.66, with a normalised action channel
of 0.136-0.248 against the healthy ~0.55 -- so pushT's -0.093 is two damaged arms compared to
each other, not a weaker version of the wall result. An earlier note here claimed P6 "reproduces
on pushT"; it does not, and the control is what says so.

| env | seed | `vel2` | `sum2` | base | `d/｜z｜` vel2 | `d/｜z｜` sum2 |
|---|---|---|---|---|---|---|
| wall | 3/4/5 | 0.24/0.06/0.18 | **0.56/0.62/0.70** | 0.68/0.60/0.74 | 0.579/0.584/0.500 | 0.582/0.652/0.579 |
| pushT | 3/4/5 | 0.00/0.00/0.04 | **0.12/0.08/0.12** | 0.24/0.38/0.66 | 0.096/0.000/1.116 | 0.214/0.248/0.136 |

**THE MECHANISM.** `vel_rel` is the objective's own success measure -- 1.0 means the head learned
nothing, 0 means it decodes motion perfectly. Against the action channel:

| run | `vel_rel` | `d/｜z｜` | SR |
|---|---|---|---|
| pushT s4 | **0.007** (near-perfect) | **0.000** (blind) | 0.000 |
| pushT s3 / s5 | 0.599 / 0.609 | 0.096 / 1.116 | 0.000 / 0.040 |
| wall s3/s4/s5 | 0.776-0.815 (barely learned) | 0.500-0.584 (healthy) | 0.06-0.24 |

**The better the velocity objective is satisfied, the more the action channel is destroyed.**
Where the head decodes motion almost perfectly the action sensitivity is exactly zero; where the
objective barely takes hold, the channel survives. P6 does not fail to help -- it fails
*because it succeeds*. Forcing the code to carry inter-frame motion competes directly with
carrying what actions do.

That also explains why P6 is untestable on pushT specifically: the objective takes hold harder
there (vel_rel 0.007-0.609 versus wall's 0.776-0.815), so it damages both arms.

n = 3 per environment, and pushT s5's `d/｜z｜` of 1.116 sits well above the inverted-U optimum
(~0.6) and is unexplained.

The wall contrast, for the record:

| | `d_action` | SR |
|---|---|---|
| `vel2` (decode the DIFFERENCE) | 0.288-0.336 | 0.060 / 0.180 / 0.240 |
| `sum2` (decode the SUM -- the control) | 0.361-0.422 | 0.560 / 0.620 / 0.700 |

Same head, same shapes, same per-target normalisation; only the content of the target differs.
Requiring the code to linearly decode inter-frame motion costs ~0.47 success, while requiring it
to decode the sum -- available from either frame alone -- is harmless. Before the repair this
contrast read `0.000 - 0.000` and carried no information at all.

**P7 is null, on BOTH environments, and it is worth showing why because it briefly looked
like the campaign's first positive.** Wall: -0.073 against both control and baseline at n = 3.
PushT completed later at **+0.133 vs control, +0.027 vs baseline** -- and per seed:

| seed | `nce2` | control | baseline | vs ctrl | `d/｜z｜` |
|---|---|---|---|---|---|
| 3 | 0.480 | 0.480 | 0.240 | **+0.000** | 0.514 |
| 4 | 0.580 | 0.160 | 0.380 | **+0.420** | 0.549 |
| 5 | 0.300 | 0.320 | 0.660 | **-0.020** | 0.465 |

The whole effect is seed 4, where the CONTROL drew 0.160 against its own 0.480 and 0.320
elsewhere -- control variance, not treatment signal (per-seed sd **0.248** against a mean of
0.133). Three independent reasons to call it null: +0.133 is half the n = 3 MDE of 0.273; the
sign flips between environments and pools to **+0.030**; and the archive's precedent is exact --
`st-ssm` read +0.083 at n = 6 and +0.000 at n = 8.

**But P7 was genuinely TESTED**, which is the point of the repair: all six runs hold normalised
action channels of 0.465-0.549, beside the baseline's ~0.549 and the inverted-U optimum near
0.6. This is a real null about the contrastive objective, not an arm that never got a fair run.

**P5 is REFUTED, not uninformative -- and correcting this exposed an error in how I was
reading the action channel throughout.**

I had been quoting the RAW `causal/d_action` and calling 0.276-0.393 "healthy". But
`train.py:1777-1806` records that the quantity which actually correlates with planning is the
SCALE-NORMALISED `causal/d_action_over_scale` = `d_action / |z|` (partial Spearman +0.545 over
235 runs), and that the relation is an **inverted U with its optimum near 0.6**, the baseline
already sitting at 0.549. Raising it is not a direction; staying in the band is.

On the raw metric `sinv2` looked degraded at 0.188. Normalised it is **0.545**, beside the
baseline's 0.619 -- its raw value was low only because its code scale is small.

| wall arm | `d_action` | `d_action/｜z｜` | SR |
|---|---|---|---|
| baseline | 0.393 | **0.619** | 0.673 |
| `sinv2` | 0.188 | **0.545** | 0.047 |
| `sinv2-shuf` | 0.324 | **0.658** | 0.050 |
| `vel2` | 0.306 | 0.554 | 0.160 |
| `sum2` | 0.383 | 0.604 | 0.627 |
| `scan2` / `st2` | 0.001 | **0.004** | -- / 0.060 |
| `enc-ssm` | 0.039 | **0.067** | 0.033 |

So **both P5 arms have healthy action channels and both plan at ~0.05 against 0.673**: the
frame-drop consistency objective destroys planning by a route that is NOT the action channel.
`sinv2-shuf` at 0.658 -- above the baseline and near the stated optimum -- with SR 0.050 is the
sharpest counterexample in the round to reading a healthy channel as evidence of health, and it
reinforces §11.4: the channel disqualifies, it never qualifies.

The catastrophic arms are unaffected by the correction: `scan2`, `st2` and `enc-ssm` sit at
0.004-0.067 on either metric.

**Corrected wall scoreboard: three readable results, not two** -- P5 refuted, P6 refuted,
P7 null, with P3/P4 still blocked by a genuinely dead action channel.

**P3 is still broken after the fix.** `scan2` mixes tokens now (verified directly), yet
`d_action` is 0.0013. The `a = 0` init was a defect but not the defect: replacing attention with
a bidirectional linear scan destroys the action channel whatever its decay. That is a finding
about the operator rather than a bug, and it means P3/P4 remain untestable as built.

---

## 12. One mechanism explains both rounds

Round 8 closed while round 9 was being repaired, and its last open question landed exactly where
the round-9 diagnosis said it would.

**ST1, the spatio-temporal tube (pushT):**

| arm | n | SR | `d_action` |
|---|---|---|---|
| `st-tube` | **8** | **0.000** | 0.062 |
| `st-tube-w5` | **8** | **0.003** | 0.026 |
| `st-tube-iid` (its matched control) | **8** | **0.000** | 0.062 |
| `blockcausal` (the substrate) | 3 | **0.000** | -- |
| `LpWM-ltv` baseline | 13 | 0.357 | 0.28-0.40 |

`st-tube` vs `st-tube-iid`: **+0.000 at n = 8** (and `st-tube-w5` +0.003). Both arms are dead, and their action channels
are 0.026-0.062 -- an order of magnitude below the healthy band. ST1 was never a test of the
tube; it was a test of `block_causal`, which had already answered.

**And that is the same disease as round 9.** Under `block_causal` a frame's code depends on all
earlier frames, so `z_{t+1}` is largely determined by `z_t` -- structurally identical to the
`s_{t+1} = A s_t + Bz x_{t+1}` shortcut in §10. The predictor lowers `z_loss` by propagating
that dependence instead of using the action, `d_action` collapses, and CEM has nothing to
optimise.

So a single mechanism now accounts for every "encoder temporal structure" failure in the
archive:

| arm | how the encoder carries information across frames | `d_action` | SR |
|---|---|---|---|
| `PiWM-blockcausal` | cross-frame attention | -- | 0.000 x3 |
| ST1 `st-tube` family | cross-frame attention (+ a tube objective) | 0.026-0.062 | 0.000 x6 |
| R9 `enc-ssm` / `enc-deep` | a linear state added to the code | 0.0009-0.128 | 0.000 |
| R9 `enc-scan` / `enc-st-scan` | attention replaced by a token scan | 0.0013 | 0.000 |
| every frozen control, and the baseline | it does not | 0.276-0.393 | 0.350-0.673 |

**The campaign-level lesson.** Three rounds have now proposed giving the encoder temporal
structure, and each time the failure was read as "the structure was not demanded by the loss"
and answered with a new objective -- T3's window summary, ST1's tube, round 9's P5-P7. The
diagnosis was wrong every time. The structure is not unused; it is used **by the predictor, as a
substitute for the action**. Adding an objective cannot fix that, because the objective acts
during training and the damage appears at planning time. What has to change is what the
predictor is allowed to see -- which is what round 9's `enc_ssm_out=aux` does, and why P6 and P7
became readable only once they stopped riding on a stateful code.

**Round 8 final scoreboard, complete at 40/40 trained and evaluated:**

| | arm | n | SR | control | contrast | `d_action` | reading |
|---|---|---|---|---|---|---|---|
| T3 | `tjepa` | 8 | 0.208 | `jump5` 0.197 | **+0.010** | **0.567** | genuinely tested -> **null** |
| T3 | `tjepa-w1` (2x dose) | 8 | 0.022 | `jump5` 0.197 | **-0.175** | 0.321 | harmful |
| ST1 | `st-tube` | 8 | 0.000 | `st-tube-iid` 0.000 | **+0.000** | 0.062 | **not testable** |
| ST1 | `st-tube-w5` | 8 | 0.003 | `st-tube-iid` 0.000 | **+0.003** | 0.026 | not testable |

The two survivors end in different places, and the distinction is the one §11.4 insists on. **T3
was genuinely tested**: its action channel is not merely intact but the healthiest in the
archive at 0.567, so its +0.010 is a real null about the window-summary objective. **ST1 was
not tested at all**: both its arms sit at 0.026-0.062, so +0.000 says nothing about the tube.

One more thing the table shows: `jump5`, T3's own control, plans at 0.197 against the baseline's
0.357. The `NUM_PRED=5` option regime costs ~0.16 on its own, so T3 was always a contrast
*inside* an already-damaged setting -- a win there would have been a win over a broken arm.

Twenty-two contrasts at n >= 8 across the campaign, still zero positives.

---

## 13. Round 9 closing scoreboard

Every contrast resolved at n = 3. `d/｜z｜` is `causal/d_action_over_scale`; the baseline sits at
~0.55-0.62 and the inverted-U optimum is ~0.6 (§ metrics). It **disqualifies** an arm from
interpretation and never qualifies one (§11.4).

**Wall** (baseline 0.673)

| | arm | SR | control | vs ctrl | vs base | `d/｜z｜` | |
|---|---|---|---|---|---|---|---|
| P5 | `sinv2` | 0.047 | 0.050 | +0.010 | **-0.627** | 0.545 | **tested -> refuted** |
| P6 | `vel2` | 0.160 | 0.627 | **-0.467** | -0.513 | 0.554 | **tested -> refuted** |
| P7 | `nce2` | 0.600 | 0.673 | -0.073 | -0.073 | 0.587 | **tested -> null** |
| P4 | `st2` | 0.060 | 0.020 | +0.040 | -0.613 | **0.004** | untestable |

**PushT** (baseline 0.357)

| | arm | SR | control | vs ctrl | vs base | `d/｜z｜` | |
|---|---|---|---|---|---|---|---|
| P5 | `sinv2` | 0.033 | 0.187 | -0.153 | -0.393 | **0.288** | untestable |
| P6 | `vel2` | 0.013 | 0.107 | -0.093 | -0.413 | 0.404 | tested |
| P7 | `nce2` | 0.453 | 0.320 | +0.133 | +0.027 | 0.509 | **tested -> null** |
| P3 | `scan2` | 0.000 | 0.007 | **+0.000** | -0.420 | **0.000** | untestable |

MDE at n = 3 is **0.273**, so nothing under ~0.27 is resolvable at this sample size.

### What the round establishes

**P5 -- refuted (wall).** Both arms hold healthy action channels and both plan at ~0.05 against
0.673. The frame-drop consistency objective destroys planning *regardless of the pairing*, so the
damage is the loss itself and not the mismatch its control was built to isolate.

**P6 -- refuted, with a measured mechanism.** Wall gives -0.467 with both arms alive. The
mechanism is the round's most specific result: `vel_rel` 0.007 (near-perfect motion decoding)
coincides with `d/｜z｜` = 0.000 (total action-blindness), while wall's barely-trained heads
(`vel_rel` ~0.8) keep healthy channels. **P6 fails because it SUCCEEDS** -- carrying inter-frame
motion competes directly with carrying what actions do.

**P7 -- null on both environments.** +0.027 and -0.073 against baseline, pooling to ~+0.03. The
+0.133 against its control was one seed of control variance (per-seed [0.000, +0.420, -0.020],
sd 0.248), and `st-ssm` already set that precedent at +0.083 (n=6) -> +0.000 (n=8).

**P3 / P4 -- untestable, definitively.** `d/｜z｜` between 0.000 and 0.004 on both environments,
and `scan2` versus its control is exactly **+0.000 at n = 3**. A bidirectional linear scan
destroys action sensitivity, and the `a = 0` init was never the cause -- that was only the first
of the two defects.

### The campaign position

**25 contrasts, still zero positives.** But three of these are **tested** nulls rather than
uninterpretable ones, which the first attempt produced none of -- every arm there died of the
substrate before its hypothesis was reached. That is what the repair bought, and it is the only
sense in which round 9 differs from rounds 6-8.

The recurring failure across three rounds is now named: an objective was mounted on an
architecture already known to plan at zero, on the theory that the architecture failed for want
of a demand. It did not. **The predictor uses the structure as a substitute for the action**, and
no training-time objective can repair a defect that appears at planning time.

---

# 14. CLOSING THE PROJECT

All jobs cancelled, all watchers stopped. Numbers frozen in `assets/round9_final.json`; figures
regenerate from that file via `analysis/round9_result_figs.py` and pass the geometric audit
(`analysis/fig_audit.py`) with zero text, shape or off-canvas collisions.

## 14.1 The scoreboard

![scoreboard](assets/2026-09-07/fig-r9-scoreboard.svg)

Every arm sits beside the control it is *defined* against, never a registered mean, and the
delta is **paired on shared seeds**. Colour carries exactly one thing: whether the arm's action
channel survived, i.e. whether its number is evidence at all.

| | wall | pushT | verdict |
|---|---|---|---|
| **P3** scan | untestable | +0.000 (n=3) | **untestable** -- channel 0.000-0.004 |
| **P4** scan+state | +0.040 | untestable | **untestable** -- channel 0.004 |
| **P5** consistency | -0.003 (both dead vs base 0.673) | -0.153 | **refuted on wall** |
| **P6** velocity | **-0.467** | -0.093 | **refuted on wall**, untestable on pushT |
| **P7** contrastive | -0.073 | +0.133 | **null, both environments** |

MDE at n = 3 is **0.273**. Nothing below that is resolvable here, which is why P7's +0.133 is a
null and not a discovery.

## 14.2 Why most of the round carries no information

![action channel](assets/2026-09-07/fig-r9-action-channel.svg)

`causal/d_action_over_scale` is how far the predicted latent moves when **only the action
changes** -- precisely what CEM ranks candidate action sequences by. Eight arm-environment cells
sit at 0.000-0.128 against a baseline of ~0.62. At that value the planner is choosing between
indistinguishable futures, and a success rate of zero is arithmetic rather than evidence.

**The rule is one-directional, and I got this wrong twice before fixing it.** A dead channel
*disqualifies* an arm from interpretation. A healthy one does **not** qualify it: `sinv2-shuf`
sits at 0.658 -- healthier than the baseline, near the inverted-U optimum of ~0.6 -- and still
plans at 0.050.

## 14.3 The one positive claim the round supports

![P6 mechanism](assets/2026-09-07/fig-r9-p6-mechanism.svg)

P6 does not fail to help. **It fails because it succeeds.** `vel_rel` is the objective's own
success measure, and where the head decodes inter-frame motion nearly perfectly (`vel_rel`
0.007) the action channel is exactly 0.000; where the objective barely takes hold (`vel_rel`
~0.8) the channel survives at 0.50-0.58. Carrying motion in the code competes directly with
carrying what actions do. Five of six runs fit; pushT s5 does not, and that is drawn rather
than dropped.

## 14.4 What the project established

**One mechanism explains every "encoder temporal structure" failure across three rounds.**
`blockcausal` (0.000 x3), ST1's whole family (0.000 x8), and round 9's P1/P2/P5-P7 all give the
predictor a way to lower `z_loss` without using the action -- cross-frame attention, or a state
added to the code, both make `z_{t+1}` largely determined by `z_t`. The predictor propagates
that instead of modelling dynamics, `d_action` collapses ~100x, and CEM has nothing to optimise.
`conf/train_rdmreg.yaml:206` had already named this failure mode before the round began.

**The campaign's recurring error was a diagnosis, not a bug.** Three rounds read each failure as
*"the loss did not demand the structure be used"* and answered with a new objective -- T3's
window summary, ST1's tube, round 9's P5-P7. The structure was never unused. It was used by the
predictor, as a substitute for the action. No training-time objective can repair a defect that
appears at planning time.

**25 contrasts at the campaign's chosen n, zero positives.** But the repaired round produced
three *tested* nulls, where the first attempt produced none -- every arm there died of its
substrate before its hypothesis was reached.

## 14.5 What I would tell the next round

1. **Read `d_action/|z|` before the success rate, and treat it only as a disqualifier.** Half
   of round 9 was uninterpretable and that was knowable from training logs alone, without a
   single CEM evaluation.
2. **Never mount an objective on a substrate that already plans at zero.** ST1 and P5-P7 both
   did, and both produced `0.000 - 0.000`. Check the substrate's own contrast first.
3. **A control must differ by one factor and remain viable.** P3's `a = 0` control did not
   weaken the scan, it deleted token mixing entirely -- twelve blocks of per-patch MLP that
   could never have encoded anything.
4. **n = 3 buys ranking, not resolution.** MDE 0.273 against a 0.357 baseline. P7's +0.133 came
   from one seed where the control drew badly.
5. **Instrument the thing you are about to spend GPU-months on, and check the instrument.** My
   own `enc_state_gap` applied the state operator twice and was quoted for days before the
   error surfaced; the raw-versus-normalised `d_action` mix-up changed a verdict from
   "untestable" to "refuted".

## 14.6 Final state

* **Queue:** empty. All watchers stopped.
* **Code:** `models/enc_state.py`, the `mup` re-apply and its RNG fix, `enc_ssm_out=aux`,
  `_encode_variant`, wall support (`ARM_ENV`, `PLAN_CFG`), evals chained at submit time.
* **Tests:** 693 passed, 1 skipped, run per file (`test_arms.py` alone exceeds the 24 GB cgroup
  limit with an unbounded cache).
* **Data:** `assets/round9_final.json`, 35 arm-environment records.
* **Figures:** three, audit-clean, regenerable.

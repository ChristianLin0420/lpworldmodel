...
### 11.4 What a null means now

If the repaired arms still fail, the failure is attributable. `d_action` is the pre-outcome read
that makes it so: a repaired arm whose `d_action` sits in the healthy 0.276-0.393 band and still
plans at zero has been genuinely tested and refuted. One below that band has not been tested at
all, and says only that the intervention broke the action channel again.

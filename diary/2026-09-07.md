...
Each figure is embedded beside the method it describes, above. Every one keeps the **house
LpWM schematic** (`arch_figs.base`) as its spine — the same `Enc f` / `ReLU` / `Pred g` /
`RDMReg` / MSE diagram every round-5..8 figure uses — and draws the proposed module **into
the pipeline** rather than describing it in a panel underneath. Below each schematic sits a
callout of **block diagrams**, not prose: the module's own dataflow, the same dataflow with
its state path cut, and the recurrence unrolled.
...
P2's state crosses lower, at y = 676, where it clears `RDMReg` (ends 608) and both observation
arrows (x = 120, x = 812).

Built by `analysis/round9_arch_figs.py`, audited by **`analysis/fig_audit.py`**, which is new
and is the reason "no overlap" is now a checkable claim. The old `round8_t_figs.audit()`
measured only `<text>`; it could not see 1,632 rects, 489 paths and 75 circles, so every
"nothing overlaps" statement in rounds 6-8 rested on a check that was blind to the shapes.
`audit_all` adds text×shape, shape×shape, and off-canvas for drawn marks, and distinguishes
**containment** (a panel holding a frame — normal) from **partial overlap** (a collision).

Two defects it caught immediately, both invisible to the old audit:

* an **arc-parsing bug of my own**: reading `d=` as flat x,y pairs treats an arc's
  `A rx ry rot large-arc sweep x y` flag triple as a coordinate, inventing a point at the
  origin. Every dome in these figures is an arc, so three phantom off-canvas marks appeared
  per figure until the parser became command-aware.
* three genuine **shape overlaps** in `arch-temporal-gap.svg` — which turned out to be
  `clip3()`'s deliberate cascade, now declarable via `allow_regions` rather than by loosening
  the tolerance everywhere.

Re-running it over the fifteen round-8 figures: **fourteen fully clean**, one declared cascade.

...
* **P1b′ (replacement).** Add **`median agent_pos_diff`** as a standing divergence diagnostic. It
  separates every healthy arm (13–28 px) from every failing one (1000–2700 px) with no overlap,
  costs nothing, and — unlike `rel_mse ≥ 0.5` — cannot be fooled by a constant-output model. Pair
...
*In practice M is capped by memory, not by the data.* `planning/ensemble.py` stacks members along
the **patch axis**, so a patch member costs `num_patches` slots where a cls member costs 1. At
M = 11 that is **11 × 256 = 2816** stacked slots against 11, and **every patch committee died with
CUDA OOM** in `planning/evaluator.py` while every cls committee ran. M = 5 (1280 slots) is the
largest that fits.
...
*(This is a real asymmetry in the ensemble implementation, not just a resource limit: stacking on
the patch axis makes a patch committee 256× more expensive in that dimension than a cls one. Any
future committee work on patch representations needs either a member axis separate from the patch
axis, or chunked scoring.)*
...
* **S2 and S4's extra seeds (11 runs).** They need n = 69 and n = 36; they were launched at
  n = 15 and n = 12.
...
* **`median agent_pos_diff > 100 px`** — a diverged rollout. Nothing in the training logs sees
  ...
  Every healthy arm sits at **13–28 px**; every failing one at **1000–2700 px**; no overlap.
...
> Detecting +0.140 needs **n = 36**; four extra seeds reach n = 12. `patchdecode` remains the

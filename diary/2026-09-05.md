...
**M is additionally capped to `min(n)` across the arms being compared** — 11 here, from
`LpWM-ltv`'s 16 finished seeds and `PiWM-columns`' 12. An unmatched M would confound committee
size with the feature, which is the same error as the unmatched latent dimension. **16 cls and
12 patch committees launched, both at M = 11.**

...
**Five corrections to this diary's own numbers are in §7**, one of which (§7.3) undercuts the
...
This is precisely what the leave-one-out design in §2c exists to fix: it yields one committee per
block, so k = n and the two levels coincide. The `loo5` runs are still queued.

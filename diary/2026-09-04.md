...
Round 6's remaining doses (`w = 0.03` at n = 3, `w = 0.3` at n = 5) are below the floor and are
not read here. `w = 0.03` is the interesting one: it is the dose that showed **no** guard cost at
all, and if it too lands negative then the objective is wrong at every strength rather than
merely over-weighted.

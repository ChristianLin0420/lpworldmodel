...
### 7.1 Every objective trades action sensitivity with its dose — except the one defined on actions
...
| family | knob ↑ | | | pattern |
|---|---|---|---|---|
| **R6 support** | 0.03 → **0.79×** | 0.1 → **0.67×** | 0.3 → **0.33×** | monotone decline |
| **R2 consist** | 0.03 → **0.72×** | 0.1 → **0.44×** | 0.3 → *pending* | monotone decline |
| **R4 incr** | 4.1e-2 → **0.25×** | 1e-2 → **0.04×** | 1e-3 → **0.00×** | monotone decline † |
| **R3 sam** | 0.01 → **0.85×** | 0.03 → **0.87×** | 0.1 → **0.74×** | **flat** |

† ε is an *inverse* strength knob — smaller ε means stronger reweighting — so R4 runs in the same
direction as the others: stronger intervention, lower action sensitivity.

Three unrelated objectives — support agreement, rollout self-consistency, and increment
reweighting — each buy their objective partly by making the predictor less responsive to the
action. They share no code path: R6 is a Jaccard on the support, R2 is a jump-vs-chain agreement,
R4 is a per-sample weight. What they share is only that **each adds a term to the latent loss that
can be satisfied by depending less on the action.**

**R3 is the exception, and it is the interesting one.** Across a 10× range of ρ its action
sensitivity does not move (0.85× → 0.87× → 0.74×, non-monotone and inside noise) and `rel_mse`
stays at baseline quality (0.011–0.014). R3 is also the *only* arm in the round whose objective is
defined **on action perturbations**: `min_θ max_{‖δa‖≤ρ} L(z, a+δa)`. An objective that explicitly
varies the action cannot be satisfied by ignoring it — the inner maximisation would find the
ignored direction. That is a mechanism, and it predicts the flatness rather than describing it.

**What would falsify this.** If R2's λ = 0.3 lands near 0.7× rather than continuing down, the
"every objective" claim breaks and it becomes a fact about R6 and R4 only. If R3 at some larger ρ
does collapse, the mechanism story is wrong. Both are decidable with runs already queued. And the
whole pattern is a statement about a *diagnostic*, not about planning: §2's lesson is that
`d_action` separates dead models from live ones and then goes flat, so a family can trade action
sensitivity and still plan fine — or hold it and still plan badly. **CEM decides, not this table.**

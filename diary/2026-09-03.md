...
The mechanism is now readable rather than guessed. Converged `val/ctrb_loss` has a median of
**0.033** against a floor of 0 for a perfectly isotropic trace-normalised Gramian — seven of the
eight seeds land between 0.022 and 0.05, with one outlier at 1.23. So the controllability Gramian
of the learned dynamics was **already close to isotropic before the penalty did anything**, and
there was almost no gradient for the term to supply. P4 is a null by construction, and this time
that claim rests on eight converged runs rather than on a number read off a partial one.

I asserted exactly this earlier in the round from a mid-training value of 0.04, had to retract it
when a fuller read gave 0.23, and it now turns out to be right at 0.033. The conclusion was
lucky; the evidence at the time did not support it. That is the same failure as the `actgain`
call above, and it is why design rule 9 exists.
...
### 16.9 V1 — voting is variance reduction, not pessimism. Levine's hypothesis is refuted.

The panel split four ways on *why* plan-time consensus is the campaign's only positive result.
V1 was built to decide one of those: if the mechanism is **pessimism**, then sharpening the
combination rule from "average the members' ranks" toward "take the worst member's rank" should
increase success monotonically.
...
All five rules are now at their full seed count. **Both halves of V1's pre-registered falsifier
fire**: `max ≤ borda`, and λ > 0 is no better than λ = 0.

Read it as an ordering rather than as five separate tests. Mean success declines monotonically
with pessimism — 0.608, 0.602, 0.585, 0.585, 0.568 — and every pessimistic rule sits at or below
both non-pessimistic ones. No individual interval excludes zero, so the honest claim is not
"pessimism hurts"; it is that **pessimism does not help, and the direction of the trend is the
reverse of what the hypothesis predicts.** `cvar1`'s [−0.055, +0.010] at n = 8 is the tightest
cell and rules out any gain worth having.

**And the positive replicates on a fresh rule and fresh seeds:** `borda` beats the single-model
baseline by **+0.215 [+0.089, +0.341]** at n = 8, against `median`'s +0.228. Whatever consensus
is doing, it is real and it is reproducible.

So the mechanism is **averaging independent model error down**, not penalising disagreement.
Which closes the loop with everything else this round measured:

- **M3**: the rollout is what is broken, not the score.
- **V2**: optimising the same objective harder through that rollout makes things *worse* — 57 %
  objective reduction, −0.077 success.
- **V1**: the only thing that has ever helped averages five independent rollouts.
- **M4**: CEM's predicted improvement beats the realised one on 38 % of episodes, and 6.7 % under
  true dynamics.

**Model error is the binding constraint of this entire project.** Four rounds of representation
and objective work were aimed elsewhere. The one intervention that works reduces model error by
averaging; the one that fails hardest (V2) exploits it harder.

That makes the remaining agenda narrow and clear: **T6 (jumpy K = 5) and T1 (attach the decoder)
are the only two proposals left that attack rollout quality directly**, and more ensemble members
is a free, already-validated axis nobody has swept. Fei-Fei's representation hypothesis also
survives in the specific form §16.3 measured — five concatenated latents decode the block twice as
well as one — but V1 shows the *combination rule* does not need to know that.

### 16.11 T7 — a near-perfect ranker that does not help, and its control says why

The energy head is trained on a **frozen** encoder to rank executed action sequences above
CEM-sampled ones, then used as the CEM leaf with no rollout at all. Its control,
`energy-distill`, is the identical architecture distilled from today's latent-MSE leaf instead.

**On the ranking task the head is excellent.** 1,536 held-out anchors, 64-way, chance 0.0156:

| scored by | top-1 |
|---|---|
| the energy head (`rank`) | **0.969 – 0.973** across all 8 seeds |
| today's CEM leaf (terminal latent MSE from the model's own rollout) | **0.029 – 0.045** |

The ordering information the planner needs **is present in the frozen `z`**. A head that never
rolls out extracts it at 97 %; the rollout-based metric CEM actually uses is at chance. That is
M3 from a fourth direction — the rollout, not the latent, is where the information is lost.

**And it does not transfer to planning.**

| arm | n | mean | vs baseline | vs its own control |
|---|---|---|---|---|
| `LpWM-ltv` | 13 | 0.357 | — | — |
| `PiWM-energy-distill` (control) | 8 | 0.265 | −0.128 [−0.191, −0.064] | — |
| `PiWM-energy` | 8 | 0.250 | −0.143 [−0.246, −0.039] | **−0.015 [−0.085, +0.055]** |

The control is what makes this readable. `energy` and `energy-distill` are **indistinguishable
from each other**, and both lose about the same amount to the baseline. So the deficit is not
caused by the ranking objective — it is caused by the thing they share: replacing a rollout-based
leaf with a learned head evaluated on CEM's own proposals. Being a 97 % ranker on the *data*
distribution buys nothing once the planner is querying its *own* distribution, which is what
model exploitation looks like when the exploited model is the score rather than the dynamics.

Two of the round's leaf-score interventions are now measured (V2, T7) and both are negative;
the third and fourth (T5, V5) are the ones still blocked on T4. Given M3, V2, V1 and now T7, I
would not expect them to be positive, and the round's remaining hope is the rollout arms — T6
above all.

### 16.13 T3 confirmed at convergence: the contact weighting collapses the model

Three seeds per arm have now finished training, so this is no longer an interim reading. The
pre-registered death condition (`rel_mse` ≥ 0.5) has fired on every converged treated seed.

| arm | converged seeds | `rel_mse` | ρ | ESS |
|---|---|---|---|---|
| `PiWM-contact` (γ = 1.0) | s3 / s7 / s8 | 0.838 / **1.0000** / **1.0000** | 0.206 / 0.039 / 0.014 | **0.38** |
| `PiWM-contact-shuf` (**control**) | s4 / s5 / s9 | 0.063 / 0.044 / 0.046 | 0.576 / 0.607 / 0.415 | **0.38** |

Median `rel_mse` **1.0000 against 0.046** — a factor of 21. Two of three treated seeds sit at
*exactly* 1.0000 with ρ collapsed to 0.014–0.039: the predictor emits the conditional mean and
the code is dead, the same signature as the four InfoNCE arms in §12.

**The effective sample size is 0.38 in both arms.** That is the whole argument. The control
carries an identically-shaped weight distribution — same ESS, same variance, same ~20× shrinkage
of the effective batch — and simply attaches those weights to *randomly chosen* transitions. It
trains normally. So the collapse is caused by **which** transitions the contact signal selects,
and by nothing else: not weight variance, not batch shrinkage, not a bug in the residual
estimator, not the learning rate.

The reading, still a hypothesis but now a well-isolated one: upweighting the ~5 % of transitions
where the block actually moves concentrates essentially all of the gradient on contact events,
which are the highest-variance transitions in the dataset. The model cannot fit them and
degenerates to the mean rather than trying. §14's rule 2 said a mean objective cannot *see* the
tail; T3 suggests the tail may also be one this model cannot *fit*, which is a materially worse
problem and one that argues against the whole family of reweighting fixes — T5's value-induced
metric included, since it reweights toward the same transitions by a different route.

T3 is therefore a clean negative with an identified mechanism, which is worth more than an
ambiguous positive would have been. The remaining seeds and the CEM numbers will land through the
autopilot, but the training story will not change.

### 16.14 T1 — attaching the decoder is a null, at either weight, with no dose response

Complete at n = 8 on all three arms. The clean contrast is against `decode-detach`, which is
identical in every config key except `decode_grad`.

| contrast | n | paired difference |
|---|---|---|
| `decode` (λ = 0.1) vs `decode-detach` | 8 | −0.060 [−0.205, +0.085] |
| `decode-w1` (λ = 1.0) vs `decode-detach` | 8 | −0.048 [−0.148, +0.053] |
| **dose**: `decode-w1` vs `decode` | 8 | +0.012 [−0.137, +0.162] |
| `decode-detach` vs `LpWM-ltv` | 8 | +0.055 [−0.085, +0.195] |

**A null at both weights, with no dose response**, and the auxiliary decoder is itself neutral.
The reconstruction gradient *is* reaching the encoder — recon loss 0.0036 with the gradient vs
0.0046 with the detach, and 0 of 144 encoder parameters receive gradient under detach against
144/144 without — so this is a null of the intervention, not of the implementation.

Together with M1 (§16.1–16.2) that closes the representation program properly. §13.4 argued that
nothing had ever forced the latent to retain visual content and that this might explain the
campaign's collapses. It was a correct description of the code and a wrong prediction about the
consequence: forcing it changes nothing that planning can use.

**This arm is also the cleanest illustration in the campaign of why n = 3 is not a result.**
The same contrast, as seeds landed:

| n | paired difference |
|---|---|
| 2 | −0.240 |
| 3 | **−0.233 [−0.39, −0.08]** ← excludes zero |
| 6 | −0.123 [−0.27, +0.03] |
| 8 | **−0.060 [−0.21, +0.09]** ← final |

At n = 3 the interval excluded zero and I noted at the time that it was thin and I had been
...
### 16.15 T3, the rest of the story: the reweighting machinery is fatal on its own

With `contact-shuf` evaluated, the two failures separate cleanly:

| contrast | n | paired difference |
|---|---|---|
| `contact` vs `contact-shuf` | 8 | −0.022 [−0.140, +0.095] |
| **`contact-shuf` vs `LpWM-ltv`** | 8 | **−0.345 [−0.476, −0.214]** |

The treated-vs-control contrast is a null **because both arms are at the floor** — `contact`
plans at 0.025 and its control at 0.048. The informative comparison is the control against the
baseline, and it is one of the largest negatives in the campaign.

So T3 fails twice, for independent reasons:

1. **The reweighting itself costs −0.345.** `contact-shuf` upweights *random* transitions at
   ESS 0.38 and loses a third of the success rate. Shrinking the effective batch ~20× is fatal
   regardless of what is upweighted.
2. **The contact selection additionally collapses training** to `rel_mse` 1.0000, ρ 0.014
   (§16.13).

The first is the more general finding and it generalises past T3: **any scheme that concentrates
the gradient on a small fraction of transitions is dead on arrival at this scale**, whatever it
concentrates on. That is a direct, measured argument against T5's value-induced metric, which
reweights toward the same tail by a different route, and against the whole "the objective can't
see the tail, so reweight toward it" family that §14's rule 2 seemed to motivate. The tail cannot
be reached by reweighting a 1-step objective. T6's option model reaches it a different way — by
changing what a step *is* — and is now the only surviving member of that family.

### 16.14 What is running
...
| V1 pessimism | 4 rules × 8 | **done** (`cvar2`/`max` at n=4/3) — pessimism refuted, positive replicated |
| M4 error analysis | from M2's traces | **done** (3 of 4 counts; contact count needs `rollout_ex`) |
| T1 | `decode`, `-w1`, `-detach` × 8 | **done** — null at both weights, no dose response (§16.14) |
| T2, T6 | `patchdecode`(±detach), `jump5`/`overshoot5` × 8 | training |
| T3 | `contact`, `-g05`, `-shuf` × 8 | **done** — collapsed (§16.13); reweighting alone costs −0.34 (§16.15) |
| T4 | `vp`, `vp-mc`, `vp-geom` × 8 | training (wave24, 96 jobs) |
| V4 | reactive policy | eval command recorded; needs a finished `PiWM-vp` |
| T7 | `energy`, `energy-distill` × 8 | **done** — −0.143 vs baseline, −0.015 vs its control |

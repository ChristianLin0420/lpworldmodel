...
**Both are now running** (`scripts/plan_consensus_controls.sh`, 50 runs): 10 at
`num_samples=1500` — exactly the M = 5 rollout count — and the 40 off-diagonal member evals, which
also make a *leave-block-out model-selection* baseline computable. That last is the honest rival to
an ensemble: if you can pick a good checkpoint from held-out blocks, you do not need five.

...
One of §12b's own points is now withdrawn: **§8 is reinstated at 66 arms and with an intervention**
([2026-09-05 §8.6](2026-09-05.md)), and the amendments to §8, §12b and §15 carry the numbers.
...
**Amended 2026-09-05.** The box is left as written, because it is the box that does not survive.
`d_action/‖z‖` has since been re-probed over 66 arms and 449 runs: **+0.624 across all arms, but
+0.316 with p = 0.142 among the 23 healthy ones**, and +0.486 (p = 0.022) among the dead ones.
§12b's +0.545 is an all-arms number read as though it were a within-healthy one, and 31 of 56
trained arms are broken. This section's conclusion — a collapse detector, not a planning lever —
is the one that holds. The numbers and the interventional test are in the addendum at the end of
this section; see also [2026-09-05 §8.6](2026-09-05.md).
...
### Addendum, 2026-09-05: the same conclusion at four times the coverage, and an intervention

`analysis/d_action_probe.py` has been run over the whole archive — **66 arms, 449 runs** — and
success recomputed on matched seeds ([2026-09-05 §8.1](2026-09-05.md)). Over the arms that carry
both quantities, split by whether the arm plans at all rather than by any threshold on the
diagnostic itself:

| subset | n arms | ρ(`d_action/‖z‖`, success) | p |
|---|---|---|---|
| all arms | 45 | +0.624 | < 0.0001 |
| **healthy, SR ≥ 0.20** | 23 | **+0.316** | **0.142** |
| dead, SR < 0.20 | 22 | +0.486 | 0.022 |

The relation lives entirely in the dead arms. That is this section's claim — *broken predictors
yes, live ones no* — restated on 66 arms instead of 71 runs from one training window, and it is
why §12b's supersession is withdrawn above.

The evidence is now interventional as well, which neither this section nor §12b had.
**`PiWM-jump2` carries the highest action sensitivity of any healthy arm — 0.980 against the
baseline's 0.549, 1.8× — and plans at −0.035 against the baseline on shared seeds.** The arms that
do win barely move the quantity: 0.90× to 1.07×, `patchdecode` (the best of them) at 0.98×. So
§12b's "optimum near 0.6" was a curve fitted across arms that differ in everything else; `jump2`
walks past it and collects nothing. Raising action influence is not a lever on a live model.
...
**Amended 2026-09-05. This point is withdrawn, and §8 stands.** +0.545 pools live and dead arms,
and 31 of 56 trained arms are broken, so the pooled figure is a collapse detector's signature.
Re-probed to 66 arms / 449 runs: **+0.316, p = 0.142 among the 23 healthy arms** against +0.486
(p = 0.022) among the dead. Point 3's optimum near 0.6 and the "validated selection statistic"
reading below inherit the withdrawal — `PiWM-jump2` sits at 0.980, 1.8× the baseline, and plans
0.035 *below* it. See §8's addendum and [2026-09-05 §8.6](2026-09-05.md). Point 1 and the ~2900×
correction itself are unaffected.
...
**Amended 2026-09-05 — T1 and T2 as run.** The rows above are left as proposed. On matched seeds
and against a *dimension-matched* reference — `PiWM-columns` (patch, no decoder) for the patch
arms, `LpWM-ltv` for the cls arms — the family reads:

| contrast, 8 shared seeds | Δ success | reading |
|---|---|---|
| **T2 `patchdecode` − `columns`** (patch, 98,304 dim) | **+0.085** | 0.520 vs 0.435; one of only two training-side arms that clear the variance bar, and an unchanged re-draw still beats it 8–13 % of the time |
| T2 `patchdecode` − `patchdecode-detach` | +0.140 | the pixel gradient pays at patch resolution |
| T2 `patchdecode-detach` − `columns` | −0.055 | the frozen decoder on its own costs |
| T1 `decode` − `decode-detach` (cls, 384 dim) | −0.060 | the same gradient **hurts** at cls |
| T1 `decode-detach` − `LpWM-ltv` | +0.055 | and this is the entire "decoder win" at 384 |

**T2 survives the variance bar; the decoder is not the mechanism.** The pixel gradient's effect
flips sign with the representation — −0.060 at cls/384, +0.140 at patch/98,304 — so "add a
reconstruction signal" names nothing that transfers between them. And at 384 the whole credit sits
on `decode-detach`, whose gradient is *provably disconnected* (`z_dec = z_emb.detach()`,
`models/visual_world_model.py:1630`), and which is measurably an **unchanged re-draw** of the
baseline: attaching a frozen decoder perturbs the world model about as much as changing the seed
does (same-seed median |rel. difference| in `effective_dim` 0.081 against the baseline's own
seed-to-seed 0.065; `mse_t1` 0.176 against 0.332), and the same-seed success correlation between
the two arms is +0.061. T1's positive is a re-draw; `decode` itself is −0.005 against the baseline
once seeds match. [2026-09-05 §8.3–§8.4](2026-09-05.md).
...
effect size, not by item, so the shape is visible at a glance.

> **CLOSED 2026-09-05: all 22 contrasts are at n = 8, none below the floor.** Final tally
> **1 positive, 5 negative, 16 null.** The table below was written mid-round and the three arms
> that were still training then have since settled — they are added at the end of it, and the
> forest figure is regenerated from live data:
>
> | contrast | n | d | 95 % CI | reading |
> |---|---|---|---|---|
> | **T2 `patchdecode` vs `-detach`** | 8 | **+0.140** | [−0.047, +0.327] | **null**, and the campaign's highest non-consensus arm mean (0.520 vs 0.357) |
> | T4 `vp-mc` vs baseline | 8 | +0.017 | [−0.163, +0.198] | null — *and this is the arm T4's own gate endorses* (`val/rho_k` 0.715; `vp` measures 0.443 and **fails** its gate) |
> | T4 `vp-geom` vs baseline | 8 | −0.015 | [−0.260, +0.230] | null |
> | T4 `vp` vs baseline | 8 | −0.100 | [−0.210, +0.010] | null |
> | **T6 `jump5` vs `overshoot5`** | 8 | **+0.005** | [−0.092, +0.102] | **a well-powered null** — see §16.10 |
> | T6 `overshoot5` vs baseline | 8 | −0.200 | [−0.387, −0.013] | negative — the 5-step *horizon* costs ~0.2 |
>
> **T4 closes as a three-way null**, and the grid is why that can be said about the arm the spec
> endorses rather than about `vp`, which a single-setting run would have tested. **T6 closes as a
> well-powered null on its own hypothesis** with a cost attached to the horizon itself. **T2 is
> the one arm that leans positive**, and `2026-09-05` §3 takes it forward.
>
> *(The forest figure used to hold these numbers as a hardcoded literal and could not see new
> evaluations — it had `jump5` frozen at n=6, `patchdecode` at n=5 and all three T4 arms under a
> "still training" header long after they finished. It is computed live now; commit `4982509`.)*

**Amended 2026-09-05.** The +0.140 is right for `patchdecode` against `patchdecode-detach` and
stands. **"0.520 vs 0.357" does not:** 0.357 is the baseline's registered 13-seed mean, while on
the eight seeds `patchdecode` actually holds the baseline averages **0.3925**
([2026-09-05 §8.1](2026-09-05.md)) — and the dimension-matched reference is `PiWM-columns` at
**0.435**, not the 384-dim baseline at all. T2's credit is **+0.085**, not 0.163. It still leans
positive, and it is one of only two training-side arms that clear the variance bar (§15,
amendment) — but the decoder is not the mechanism behind it.
...
### 16.8b Reanalysis: round 5 with the success-rate structure put back

*(added 2026-09-05. Method and the accompanying retraction: `diary/README.md` §7.)*

The same treatment applied to round 5. Reporting the rate at which each arm returns a non-zero
success rate, beside the mean:

| contrast | Δ mean SR | seeds with SR>0 (T/C) | McNemar p |
|---|---|---|---|
| **V1 consensus** | **+0.228** | **10 / 10** | 1.000 |
| T2 `patchdecode` | +0.140 | 8 / 7 | 1.000 |
| T4 `vp-mc` | +0.017 | 8 / 8 | 1.000 |
| T6 `jump5` vs `overshoot5` | +0.005 | 7 / 7 | 1.000 |
| T1 `decode` | −0.060 | 8 / 8 | 1.000 |
| V2 `gd` | −0.077 | 8 / 8 | 1.000 |
| T6 `overshoot5` vs base | −0.200 | 7 / 8 | 1.000 |
| T3 `contact-shuf` vs base | −0.345 | **3 / 8** | 0.062 |

**Round 5's readings are almost all rate-preserving**, which strengthens them. Consensus adds
+0.228 while every one of ten seeds returns a score on both sides — it is not winning by avoiding
zeros. T6's null is a null on arms that both return scores on 7 of 8. `overshoot5`'s −0.200 comes
with 7/8 versus 8/8, so the 5-step horizon genuinely costs planning quality rather than costing
trainability.

The single exception is **T3's `contact-shuf`**, which returns a non-zero score on 3 of 8 seeds
against the control's 8 of 8. §16.0 already flagged that both T3 arms sit at the floor and that
the T3 contrast is uninformative; the rate column says the same thing from the data rather than
from a threshold.

**Consensus is the one result in the campaign that is unambiguous on every reading available**:
paired mean, compute-matched control, leave-block-out selection, and now the rate of returning a
score. §16.9 is the full audit.

*Caveat on the rate column, established the same day it was added (`README` §7): P(SR > 0) is a
description of the score distribution, **not** evidence that a model trained. In two round-6 arms
it is anti-correlated with model health — a collapsed constant-output model counts as "working"
on one success in fifty episodes. It carries information here only because round 5's arms are
almost all rate-preserving, so nothing hinges on interpreting it.*
...
#### The compute-matched control landed. Consensus is not a compute story.

The last objection was the strongest: the ensemble spends **4.3× plan-time wall-clock** and no
compute-matched control existed anywhere in 549 archived plan runs. An M = 5 committee does M
rollouts per candidate × 300 candidates = **1500 rollouts per optimisation step**; one model at
`num_samples=1500` does exactly the same. Both directions of that knob were run, 10 seeds each,
same checkpoints, same episode blocks, nothing varying but the budget.

| planner budget | n | mean |
|---|---|---|
| 60 samples (0.2×) | 10 | 0.354 |
| 300 samples (baseline) | 10 | 0.374 |
| 1500 samples (5×) | 10 | 0.370 |

**Flat across a 25× range.**

| contrast | n | Δ | 95 % CI | p |
|---|---|---|---|---|
| 1 model @ 5× vs 1 model @ 1× | 10 | −0.004 | [−0.066, +0.058] | 0.887 |
| **M = 5 ensemble vs 1 model @ 5× (compute-matched)** | 10 | **+0.232** | **[+0.130, +0.334]** | **0.0006** |
| M = 5 ensemble vs 1 model @ 1× (the headline) | 10 | +0.228 | [+0.130, +0.326] | 0.0005 |

**Giving one model exactly the ensemble's rollout budget buys nothing (−0.004); giving it five
models buys +0.23.** The compute-matched contrast is if anything *larger* than the headline. The
4.3× cost objection is answered: the compute is not what produces the gain.

Two other things fall out of the same runs. **CEM is not search-limited** — 25× more or less
search changes nothing, so the planner is far past the point where its budget binds. And the
**over-optimisation hypothesis is dead**: if the planner were exploiting its model's errors, a
weaker maximiser should have helped and a stronger one should have hurt. Neither happened.

#### Where that leaves the campaign's one positive

| consensus vs… | Δ | reading |
|---|---|---|
| its members' **mean** | +0.222 [+0.177, +0.268] | solid |
| a **compute-matched** single model | **+0.232 [+0.130, +0.334]** | **solid** |
| a **leave-block-out selected** single model | +0.072 [−0.001, +0.145] | marginal, p = 0.052 |
| its members' **best**, same block | +0.058 [−0.008, +0.124] | marginal, p = 0.076 |

The honest statement is now narrow but real: **the gain comes from having several independently
trained models, not from compute and not from search.** §7.3 of `2026-09-04` measured the
mechanism directly — the members' errors are independent (ρ̄ ≈ 0.001 at K = 1, −0.001 at K = 5)
and averaging removes them at the theoretical 1/√M rate. What caps the gain is not correlation
between members but the quality of the best member: where one member is genuinely strong the
committee only ties it, and where none is, the committee beats them all.

That is a real result and it is the only one this campaign has. It is also, notably, **not a
world-model result** — it says nothing about the objective, the representation or the rollout,
which is what six rounds of arms were about.
...
> **Update 2026-09-05 — this section's expectation for R1 was right, but I nearly mis-read the
> evidence.** R1's `jump` vs `overshoot` contrast leans positive at K = 2 (+0.167) and K = 3
> (+0.135) and I twice flagged that as evidence *against* the expectation recorded here. Adding
> the baseline column shows the opposite: `jump` is **−0.035** against the baseline at K = 2 and
> **−0.264** at K = 3, while `overshoot` is −0.220 and −0.372. The jump never beats one-step
> training at any K; the contrast is positive only because the control degrades, and
> `overshoot`'s `effective_dim` falls to 10.6 / 7.8 against the baseline's ≈ 24. See
> `2026-09-04` §7.0c.

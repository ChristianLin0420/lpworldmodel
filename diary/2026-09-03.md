...
## 16. Round 5, first results: M1 answers its question, and the answer is the opposite one

Round 5 launched the same day. Three measurement items and three planner arms are already on the
cluster; the training arms follow. This section holds what has landed.

### 16.1 M1 — the latent encodes the block, and that is bad news for the representation program

The probe ridge-regresses the **frozen** latent to the block's position and orientation across all
328 checkpoints, split by episode, with five controls. Median held-out error over the arms that
both predict and plan:

| what is decoded from | block position | block angle |
|---|---|---|
| **`z`, pre-link** | **14.7 px** | **14.0°** |
| **`z`, post-link** (what the predictor consumes) | 18.4 px | 18.2° |
| agent pose alone | 56.4 px | 25.5° |
| a random, never-trained encoder | 65.5 px | 26.6° |
| a constant predictor | 80.2 px | 21.6° |
| shuffled labels | 84.9 px | 23.2° |

![M1 latent probe](assets/2026-09-03/m1-probe.png)

**The latent beats every control by a wide margin on position.** The agent-pose control is the one
that mattered — the agent is usually touching the block and carries 90.9 % of the initial
`pos_diff²` — and `z` beats it 3.1×. So the answer to "does the latent contain the block at all"
is an unambiguous yes.

**The link is destroying information.** Pre-link `z` decodes the block 20 % better in position and
4.2° better in angle than the post-link code — and the post-link code is the one the predictor and
the planner actually consume. That is a free, actionable finding: `reprelu` is discarding task
content, and nothing in the objective penalises it for doing so.

### 16.2 The finding I did not expect: the best representations belong to the worst planners

| arm | block position | block angle | CEM |
|---|---|---|---|
| `LpWM-linvar` | **9.5 px** | **4.4°** | 0.090 |
| `PiWM-ctrb` | 10.5 px | 5.8° | 0.070 |
| `PiWM-multact` | 14.6 px | 6.1° | 0.060 |
| `PiWM-lie` | 15.3 px | 6.8° | 0.020 |
| `PiWM-lie-sim` | 16.3 px | 7.3° | 0.020 |
| `PiWM-columns` | 14.8 px | 13.3° | **0.490** |
| `LpWM-ltv-d2048` | 17.9 px | 18.1° | **0.690** |
| `LpWM-ltv` (baseline) | 19.4 px | 19.4° | 0.380 |

`LpWM-linvar` localises the block **twice** as accurately as the baseline and its orientation
**4.4×** as accurately — 4.4° against a task tolerance of 20° — and plans at 0.090 against 0.380.
Every one of the five best representations in the campaign is a linear-predictor arm, and every
one of them is among the worst planners.

This is the dissociation round 4 was designed to find, discovered on a different axis. A latent
can encode the task almost perfectly and still be unplannable. **Representation quality is not
what the ReLU is buying**, and the representation program was not merely unproductive — it was
aimed at a variable that is already saturated.

There is a mechanism worth stating as a hypothesis, not a finding: a linear predictor can only
exploit linear structure, so it pushes the encoder toward a code in which the scene is *linearly*
laid out — which is exactly what a ridge probe rewards. The baseline's ReLU predictor lets the
encoder keep a code that is worse by that measure and better for planning.

Two caveats. The relation *within* a predictor class is the expected one — Spearman(position
error, CEM) is −0.73 among nonlinear arms and −0.42 among linear ones, both negative, i.e. a
better probe does go with better planning at fixed class. The reversal is entirely *between*
classes, which is a textbook confound structure and means "improve the probe" remains a plausible
direction *within* the baseline's class. And the probe is linear; a nonlinearly-decodable code
would be scored unfairly.

### 16.3 M3 — the oracle ladder, two cells of four

Three seeds per cell, on `LpWM-ltv` checkpoints:

| | learned dynamics |
|---|---|
| **learned objective** | 0.22, 0.32, 0.56 → mean **0.37** |
| **oracle objective** | 0.16, 0.06, 0.28 → mean **0.17** |

Replacing the learned leaf score with the true task distance makes planning **worse**, not better.
I am not going to interpret this until the other two cells land and I have read the
implementation — an oracle objective evaluated on *predicted* latents is not the same experiment
as an oracle objective evaluated on true states, and which of those was run decides what the cell
means. Flagged here because it is the single most surprising number in the round so far.

### 16.4 What is running

| | arms | seeds | status |
|---|---|---|---|
| M1 probe | 328 checkpoints, 5 controls | — | **done** |
| M2 metric audit | rescore of the archive | — | running |
| M3 oracle ladder | 4 cells | 3 | 2 of 4 cells |
| V1 pessimism | `vote5-borda`, `-cvar1`, `-cvar2`, `-max` | 8 | launched, 32 evals |
| V2 gradient | `gd`, `gd-noise0` | 8 | launched, 16 evals |
| V3 two-level | `2lvl` | 8 | launched, 8 evals |
| T1–T3, T6 | decoder, patch+decoder, contact, jumpy | 8 | implementing |
| T4, V4 | value head, reactive policy | 8 | queued behind T1–T6 |
| T7 | energy head | — | queued |
| T5, V5 | value-equivalent, value-guided | — | **blocked**: need a trained T4 head |
| M4 | error analysis | — | **blocked**: needs M2's per-episode traces |

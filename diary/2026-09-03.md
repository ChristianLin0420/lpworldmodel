...
### 16.13 T3, in flight: the contact weighting is collapsing, and its control says why

`PiWM-contact` is at epoch 1–2 of training, so this is an interim reading, but the contrast with
its own control is already unambiguous and the pre-registered death condition (`rel_mse` ≥ 0.5)
has fired on 8 of 8 seeds.

| arm | weights | `rel_mse` across 8 seeds | ESS | `w_max` |
|---|---|---|---|---|
| `PiWM-contact` (γ = 1.0) | proprio residual | **0.68 – 1.00** | 0.33–0.45 | 3.5–6.2 |
| `PiWM-contact-g05` (γ = 0.5) | proprio residual, softened | **0.42 – 1.00** | 0.42–0.56 | 2.7–3.9 |
| `PiWM-contact-shuf` (**control**) | the *same weights*, shuffled across the batch | **0.018 – 0.089** | 0.33–0.45 | 4.0–5.6 |

Two seeds sit at `rel_mse` exactly **1.0000** with ρ = 0.018 — the collapse signature from §12,
where the predictor emits the conditional mean and the code goes dead.

**The shuffled control is what makes this interpretable, and it is why it was mandatory.** It
carries the identical weight distribution — same effective sample size, same maximum weight — but
attaches those weights to *random* transitions. It trains completely normally, at `rel_mse` an
order of magnitude lower, at the same epoch. So the failure is **not** weight variance, not the
~20× shrinkage of the effective batch, and not a bug in the estimator. It is specifically *which*
transitions the contact signal selects.

The reading, offered as a hypothesis: upweighting the 5 % of transitions where the block actually
moves concentrates nearly all the gradient on the most stochastic events in the dataset —
contacts, where outcome variance is highest — and the model degenerates rather than fitting them.
The tail that §14's rule 2 says the objective cannot see may be a tail the objective cannot *fit*
either.

If this holds at convergence it is a genuine negative result with a clean mechanism rather than a
failed implementation, which is worth more than the arm succeeding would have been ambiguous. I
am letting all three arms run to completion — a measured CEM number with its control beats a
saved GPU-hour, and the arm is failing rather than broken.
...
| T1, T2, T6 | 7 arms × 8 seeds, each with its own control | training (wave23) |
| T3 | `contact`, `-g05`, `-shuf` × 8 | training — **death condition fired on 8/8, see §16.13** |

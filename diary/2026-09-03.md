...
The round's actual product is §8 — a **conjunctive gate on three training-time diagnostics
that so far separates every planning run from every non-planning one at n = 54, p = 5e-8**,
while each of the three alone is far weaker.
...
code stacks, dark-red MSE. Each diagram adds **only** the proposed part, in its accent colour,
with a dashed callout carrying the measured outcome.
...
## 8. The round's real result: no single diagnostic predicts planning, the conjunction does

This is what the per-run instrumentation bought, and it is worth more than any of the three
interventions.
...
**Every run that passes all three plans above zero; there are no exceptions.** The conjunction
is four orders of magnitude stronger than any single condition. Per-run
...
**What this is not.** The gate is **sufficient so far, not necessary** — nine gate-failing runs
still plan above zero, three of them meaningfully:

| run | CEM | fails on |
|---|---|---|
| `PiWM-actinfo` s7 | 0.32 | rel_mse only (0.0674, just over the line) |
| `PiWM-actinfo` s9 | 0.16 | ρ (0.051 — genuinely collapsed, and it plans anyway) |
| `PiWM-incr-actinfo` s3 | 0.06 | ρ and rel_mse |

And the thresholds were **chosen by inspection after seeing these runs**, so this is a
descriptive separation, not a pre-registered test. The 18 queued evals are the out-of-sample
check, and the honest prediction to hold it to is: *gate-passing runs should keep planning above
zero, and the mean CEM gap should stay near 0.3.*
...
- **Whether the gate is causal or descriptive.** Its thresholds were fit to these runs. The 18
  queued evals decide it.

...
| contrast | n | paired difference |
|---|---|---|
| `contact` vs `contact-shuf` | 7 | −0.026 [−0.166, +0.114] |
| **`contact-shuf` vs `LpWM-ltv`** | 7 | **−0.340 [−0.496, −0.184]** |

The treated-vs-control contrast is a null **because both arms are at the floor** — `contact`
plans at 0.025 and its control at 0.054. The informative comparison is the control against the
baseline, and it is one of the largest negatives in the campaign.

So T3 fails twice, for independent reasons:

1. **The reweighting itself costs −0.34.** `contact-shuf` upweights *random* transitions at

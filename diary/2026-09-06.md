...
| fact | source |
|---|---|
| the encoder encodes every frame **independently** — *"no temporal structure whatsoever"* | `visual_world_model.py:1234-1237` |
...
| `num_hist` = 3 and `frameskip` = 5 in **all 400** sampled run configs | run configs |
| `detach_target: False`, so the target back-propagates into itself — **this was never a JEPA**; **no EMA teacher exists** | `visual_world_model.py:36, 1129, 1456, 581` |

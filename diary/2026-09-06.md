...
Fifteen arms launched. Six proposals did not, and this section is each of them: what it is,
how it is built, what would falsify it. Three are implemented with a canary queued; three are
**gated on M2** and deliberately unbuilt, because M2 decides whether ~500 GPU-h of
metric-shaped proposals are worth spending at all.
...
### 4.4 S2 · `PiWM-wscore` — the plan-time metric ✦ GATED, and it is the gate
...
a synthetic code with the measured anisotropy. **Not launched** — it needs the eval sbatch to
pass `+objective.wmat_path`, and that plumbing is worth verifying before it runs.
**GATE: below +0.05, S2, ST4 and ST5 are all withdrawn.**

### 4.5 ST4 · `PiWM-st-metric` — one weight over DIRECTION and HORIZON ✦ GATED
...
### 4.6 ST5 · `PiWM-st-goal` — a goal pooled over SPACE, scored over TIME ✦ GATED
...
have **0 occurrences**; the only objective override ever used is the vote family. Plan-time
only ⇒ same-checkpoint control, free, bar +0.05.
...
| **GPU path (DDP, bf16, real data)** | **NOT verified** — all 8 canaries are still `PENDING` |
| full suite | **658 passed, 1 skipped** (1080 s) against the final code |
